...
void calcula(std::string positions, std::string input_file, std::vector<double> &v1, std::vector<double> &v2,
  ...
  for(int n=0; n!=tree->GetEntries(); ++n){
    ...
    if (numberDevices > kMaxDevices) {
      std::cerr << "ERROR: numberDevices=" << numberDevices
                << " exceeds kMaxDevices=" << kMaxDevices
                << " (array overflow risk). Skipping entry " << n << "\n";
      continue;
    }
