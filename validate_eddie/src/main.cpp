...
// DUNE-VD Gaisser-Hillas
  ...
  const double fGHVUVPars[4][9] = { {1.04201, 1.02373, 0.988003, 0.923913, 0.829408, 0.738727, 0.628257, 0.507532, 0.369859},
                                    {114.312, 152.358, 138.69, 152.845, 170.084, 191.901, 210.986, 238.175, 240.508},
                                    {10, 10.7966, 13.7449, 14.53, 16.9071, 47.4787, 87.3491, 97.1228, 152.855},
                                    {-5000, -3000, -3000, -3000, -3000, -1000, -500, -500, -100}};

  const std::vector<double> slopes1 = {0.000365862, 0.000404818, 0.000391108, 0.000361403, 0.0003313, 0.000281984, 0.000235596, 0.000197912, 0.00015956};
  const std::vector<double> slopes2 = {0.0295673, 0.027927, 0.0500086, 0.0493777, 0.0430743, 0.039966, 0.00961879, 0.00201935, -0.00346761};
  const std::vector<double> slopes3 = {9.58835e-10, -0.00115982, -0.00408358, -0.00460647, -0.00564017, -0.0167624, -0.0196656, -0.0266828, -0.0451904};
  // double_sided
...
int main(int argc, char * argv[]){
  ...
  if(myfile.is_open()) {
    ...
    int min_id = 1e9, max_id = -1e9;
    for (const auto& row : optical_detector_positions) {
      int id = static_cast<int>(row[0]);
      if (id < min_id) min_id = id;
      if (id > max_id) max_id = id;
    }
    std::cout << "Loaded ID range: " << min_id << " to " << max_id << std::endl;
  ...
  for(int n=0; n < tree->GetEntries(); n++) {
    ...
    if (fVerticalBorderCorrectionMode) {
        // vertical-only border correction
        R = std::abs(posSource[1] - y_foils);
    }
    else {
        // original radial distance in YZ plane
        R = std::sqrt(
            std::pow(posSource[1] - y_foils, 2) +
    ...
    v_hits_sim.push_back(VUV_hits[nPMT]);  // full simulation visible hits
...
int VUVHits(const int &Nphotons_created, const TVector3 &ScintPoint, const TVector3 &OpDetPoint, const int &optical_detector_type, const double &cosine, const double &theta, const double distance, const int &j) {
  ...
  if (fVerticalBorderCorrectionMode) {
    r_distance = std::abs(ScintPoint[1] - y_foils);
  }
  else {
    if (op_channel_orientation == 0)
      r_distance = std::sqrt(std::pow(ScintPoint[1] - y_foils, 2) +
                            std::pow(ScintPoint[2] - z_foils, 2));
    else if (op_channel_orientation == 1)
      r_distance = std::abs(325.01 - ScintPoint[0]);
  }
